...
// Accepts any Vimeo link format and returns the player URL:
//   vimeo.com/123456789            -> player.vimeo.com/video/123456789
//   vimeo.com/123456789/abcdef1234 -> player.vimeo.com/video/123456789?h=abcdef1234
// Other URLs (player links, Bunny Stream...) are used as they are.
export function embedUrl(value) {
  ...
  const match = /^(?:www\.)?vimeo\.com$/.test(url.hostname) && /^\/(\d+)(?:\/([0-9a-f]+))?\/?$/.exec(url.pathname);
  if (!match) return value;
  const player = new URL(`https://player.vimeo.com/video/${match[1]}`);
  if (match[2]) player.searchParams.set('h', match[2]);
...
export async function filmSource() {
  ...
  if (embed) return { kind: 'embed', src: embedUrl(embed) };
  return { kind: 'none' };
}
