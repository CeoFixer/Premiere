...
import { HttpError, assertSameOrigin, clientIp, handle, json, rateLimit, readJson } from '../lib/http.js';
import { stripe } from '../lib/stripe.js';

// POST /api/checkout  { item: "film" }  or  { item: "donation", amount: 1000 }
...
export const POST = handle(async (request) => {
  ...
  if (body.item === 'film') {
    ...
    params = {
      ...
      metadata: { item: DONATION.id },
      payment_intent_data: { metadata: { item: DONATION.id }, description: DONATION.title },
      submit_type: 'donate',
      success_url: `${site}/thank-you?session_id={CHECKOUT_SESSION_ID}`,
