...
Либо закрытый плеер — **так фильм сейчас работает на Wix (Vimeo)**: `FILM_EMBED_URL=https://vimeo.com/123456789/abcdef1234` (подходит любая ссылка Vimeo, сайт сам превратит её в ссылку плеера). Сайт отдаёт эту ссылку только после проверки оплаты. В Vimeo: видео → **Settings → Privacy → Where can this be embedded? → Specific domains** — добавьте `fixer-premiere.com` и `www.fixer-premiere.com` (для проверки до запуска — ещё адрес превью `*.vercel.app`), а в **Who can watch** оставьте «Hide from Vimeo». Это слабее, чем бакет: адрес плеера одинаковый у всех покупателей, защищает именно ограничение по домену.
